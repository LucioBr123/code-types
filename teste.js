...
inputElements.forEach(input => {
  input.addEventListener('input', async () => {
    textoTela.textContent = textoTela.textContent
    await new Promise(resolve => setTimeout(resolve, 500));
    const textoArray = String(input.value).split('') 
    const textoTelaArray = String(texto).split('')
    erro = []
    ...
    console.log(`
    Texto digitado:${textoArray}--${textoArray.type}
    TextoFundo ${textoTelaArray}--${textoTelaArray.type}
    indice dos erros: ${erro}--${erro.type}
    `)
  });
});
...
// armazena na variavel o que ta errado da comparaçõa 
